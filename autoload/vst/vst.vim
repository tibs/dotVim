...
" Prevent multiple loading of VST, how it works with autoload?
"if exists('b:vst_loaded')
"	finish
"endif
"let b:vst_loaded = 1
"
...
function! VST_Headers(text)
	...
	for i in range(len(doc))
		...
		elseif doc[i] =~ '^\s*[*+-]\s'
			...
			if nind > cind 
				let ltype += ['uli']
			elseif nind == cind && doc[nextline] =~ '^\s*[*+-]\s'
				let ltype += ['uli']
			elseif doc[nextline] =~ '^\s*$'
				let ltype += ['uli']
			else
...
while i < len(g:paras)
	if g:ptype[i] == 'uli'
		...
		let style = matchstr(g:paras[i], '^\s*\zs[+*-]\s*')
		let listindent = strlen(matchstr(g:paras[i], '^\s*\zs[+*-]\s*'))
		if style =~ '+'
			let ustyle = 'square'
		elseif style =~ '\*'
			let ustyle = 'circle'
...
function! VST_CreateDBs(table)
	...
	let dbtable = filter(deepcopy(preproc), 'v:val =~ "^\\s*\\.\\. \\(_\\||\\|role\\)"')
	for i in range(len(dbtable))
		" Remove common prefix
		let dbtable[i] = substitute(dbtable[i], '^\s*\.\. ', '', '')
		if dbtable[i] =~ '^_'
			" Hyperlink database
			let title = tolower(matchstr(dbtable[i], '^_\zs.\{-}\ze:'))
			let url = matchstr(dbtable[i], '^_.\{-}:\s*\zs.*')
			if url =~ '@' && url !~ '^http\|^ftp'
				let url = 'mailto:'.url
			endif
			if title != '' && title != '_'
		...
		elseif dbtable[i] =~ '^role'
			let rolekey = matchstr(dbtable[i], '^role::\s*\zs.\{-}\ze\s*-vst-role-')
			let rolevalue = matchstr(dbtable[i], '-vst-role-\s*:class:\s*\zs.*\ze\s*$')
			if rolevalue == ''
...
function! VST_Hyperlink(text)
	...
	if par =~ '__'
		...
		let par = substitute(par, "[- '\"([{</:>]\\@<=`\&lt;\\(.\\{-1,}\\)\&gt;`__[\\]<\\- '\")}>/\\:\\.,;!?]\\@=", '<vim:a href="\1" title="\1">\1</vim:a>', 'g')
		let par = substitute(par, "[- '\"([{</:>]\\@<=`\\([^[:space:]`]\\)\\([^`]\\{-}\\) \&lt;\\(.\\{-1,}\\)\&gt;`__[\\]<\\- '\")}>/\\:\\.,;!?]\\@=", '<vim:a href="\3" title="\1\2">\1\2</vim:a>', 'g')
	...
	" Embedded hyperlinks
	let par = substitute(par, "[- '\"([{</:>]\\@<=`\&lt;\\([^`]\\{-1,}\\)\&gt;`_[\\]<\\- '\")}>/\\:\\.,;!?]\\@=", '<vim:a href="\1" title="\1">\1</vim:a>', 'g')
	let par = substitute(par, "[- '\"([{</:>]\\@<=`\\([^[:space:]`]\\)\\([^`]\\{-}\\) \&lt;\\(.\\{-1,}\\)\&gt;`_[\\]<\\- '\")}>/\\:\\.,;!?]\\@=", '<vim:a href="\3" title="\1\2">\1\2</vim:a>', 'g')
...
function! VST_SA_Hyperlink(text)
	...
	let par = substitute(par, '&lt;\(https\?://[a-zA-Z0-9./%&@#:;?=_-]\+\)&gt;\(`_\)\@!', '\="\&lt;<vim:a href=\"".VST_PunctTrap(submatch(1))."\">".submatch(1)."</vim:a>\&gt;"', 'g')
	let par = substitute(par, '&lt;\(s\?ftp://[a-zA-Z0-9./%&#@:;?=_-]\+\)&gt;\(`_\)\@!', '\="\&lt;<vim:a href=\"".VST_PunctTrap(submatch(1))."\">".submatch(1)."</vim:a>\&gt;"', 'g')
	let par = substitute(par, '&lt;mailto:\([a-zA-Z0-9@&#.;?=_-]\+\)&gt;\(`_\)\@!', '\="\&lt;<vim:a href=\"mailto:".VST_PunctTrap(submatch(1))."\">".submatch(1)."</vim:a>\&gt;"', 'g')

	let par = substitute(par, '\(href="\|&lt;\)\@<!https\?://[a-zA-Z0-9./%&@#:;?=_-]\+', '\="<vim:a href=\"".VST_PunctTrap(submatch(0))."\">".submatch(0)."</vim:a>"', 'g')
	let par = substitute(par, '\(href="\|&lt;\)\@<!s\?ftp://[a-zA-Z0-9./%&#@:;?=_-]\+', '\="<vim:a href=\"".VST_PunctTrap(submatch(0))."\">".submatch(0)."</vim:a>"', 'g')
	let par = substitute(par, '\(href="\|&lt;\)\@<!mailto:\([a-zA-Z0-9@&#.;?=_-]\+\)', '\="<vim:a href=\"".VST_PunctTrap(submatch(0))."\">".submatch(2)."</vim:a>"', 'g')
...
" }}}
	...
	while i < len(text)-1
		...
		if text[i] =~? '^\s*\.\. \(note\|tip\|warning\|attention\|caution\|danger\|error\|hint\|important\|admonition\)::\s*$' && text[i+1] =~ '^\s*[*+-]\s'
			let findent = strlen(matchstr(text[i], '^\s*'))
			let sindent = strlen(matchstr(text[i+1], '^\s*'))
		...
		if format == 's5'
			...
			for file in s5files
				if !filereadable('s5ui/'.file)
					...
					exe 'silent below 1split '.s5file
					exe 'silent write! '.workingdir.'s5ui/'.file
					exe 'silent bw! '.s5file
				endif
			endfor
			" Write file after changes
			"silent write
			exe 'silent cd '.workingdir

		endif
		" }}}
		if exists("g:vst_write_export") && g:vst_write_export != 0
			silent exe 'write! '.filename.'.html'
		endif
		" User postprocessing
		if g:vst_html_post != '' && filereadable(g:vst_html_post)
			exe "silent! source ".g:vst_html_post
		endif
		silent call cursor(1,1)
	...
	elseif format =~ 'tex\|pdf'
		...
		"
		" Rubric
		let file = substitute(file, '<vim:rubric.\{-}>', '{\large ', "g")
		...
		if format =~ 'pdf'
			...
			silent exe 'write! '.file_tex
			silent exe "call system(\"".g:vst_pdf_command." '".file_tex."'\")"
			if search('\\tableofcontents')
				silent exe "call system(\"".g:vst_pdf_command." '".file_tex."'\")"
			endif
			if v:shell_error == 0
				bw! %
	...
	elseif format =~ '^fold'
		...
		" Ancient fold systems (worth of preserving): {{{
